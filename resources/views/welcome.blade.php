...
<body>
    ...
    <div class="container mt-5 hero-section">
        <div class="hero">
            ...
            <a href="{{ route('recipes.create') }}" class="btn btn-accent rounded-[60px] px-4 btn-lg mt-4 font-semibold">Share a Recipe</a>
        </div>
    </div>
    ...
    <!-- Top 3 Rated Recipes -->
    @if(isset($topRecipes) && count($topRecipes) > 0)
    <div class="container my-5">
        <h4 class="text-center mb-4">
            <span class="top-rated-badge">🏆 Top 3 Rated Recipes</span>
        </h4>
        <div class="row g-4">
            @foreach ($topRecipes->take(3) as $recipe)
                <div class="col-md-4">
                    <div class="card h-100 shadow-sm">
                        <img
                            src="{{ asset('storage/' . ($recipe->images->first()->path ?? 'images/default.jpg')) }}"
                            class="card-img-top"
                            alt="Recipe Image"
                        />
                        <div class="card-body">
                            <h5 class="card-title">{{ $recipe->title }}</h5>
                            <p class="card-text">Submitted by {{ $recipe->user->name }}</p>
                            <a href="{{ route('recipes.show', $recipe->id) }}" class="btn btn-accent btn-sm rounded-[60px]">View Recipe</a>
                        </div>
                    </div>
                </div>
            @endforeach
